...
def updateNonHumanWatchList(pdef: ET.Element):
    ...
    if raceEle != None and raceEle.text != None and raceEle.text not in nonHumanExceptionList:
        ...
        print(nameOrDefName + ' with race ' +
              raceEle.text+' added to non human watchlist')
...
def createPKD(
    ...
    for file in sourcePawnKindDefsFileNames:
        outputFiles[file] = {'abstracts': ET.Element(
            'Defs'), 'nonabstracts': ET.Element('Defs')}
        pawnKDTree = None
        if sourceModID == 'Core':
            fileName = getRimworldDataModsPath()+'\\'+sourceModID+'\\'+file+'.xml'
        elif sourceModID == 'Ideology':
            fileName = getRimworldDataModsPath()+'\\'+sourceModID+'\\'+file+'.xml'
        elif sourceModID == 'Royalty':
            fileName = getRimworldDataModsPath()+'\\'+sourceModID+'\\'+file+'.xml'
        else:
        ...
        if pawnKDTree == None:
            print(file)
            raise 'YOUFUCKEDUP file did not import'
        root = pawnKDTree.getroot()
        for pawnKindDef in root.findall('PawnKindDef'):
            nameOrDefName, defNameValue, attribName = getNameOrDefName(
                pawnKindDef)
            attribAbstract = pawnKindDef.attrib.get('Abstract')
            if attribAbstract == 'True':
                # print('found abstract pawnkind')
                # print(pawnKindDef.attrib)
                possibleParents[attribName] = pawnKindDef
            else:
                if attribName != None:
                    possibleParents[attribName] = pawnKindDef
                # uncomment for some log spam
                print('non abstract pawnkind found')
                print(pawnKindDef.attrib)
                print(defNameValue)
                print(file)
                pawnKindsFound[nameOrDefName] = {
                    'element': pawnKindDef, 'filename': file}
    for key in pawnKindsFound.keys():
        ...
        newPKD.append(age)
        if pawnKindsFound[key]['filename'] in outputFiles.keys():
            ...
            for speciesEntry in speciesArray:
                outputFiles[pawnKindsFound[key]['filename']]['nonabstracts'] = createSpeciesFile(speciesEntry, newPKD, labelText, nameOrDefName, defsClass,
                                                                                                 outputFiles[pawnKindsFound[key]
                                                                                                             ['filename']]['nonabstracts'],
                                                                                                 destinationModPrefix, includeBackstories)
...
def createSpeciesFile(speciesEntry, newPKD: ET.Element, labelText, nameOrDefName, defsClass, SPECIESDEFS, destinationModPrefix, includeBackstories: bool):
    for race in speciesEntry['race']:
        pawnKindBeingCreated = ET.Element('PawnKindDef', {
            'ParentName': newPKD.attrib.get('Name')})
        ele = ET.Element('race')
        ele.text = race
        ...
        if 'apparelTags' in speciesEntry and len(speciesEntry['apparelTags']) > 0:
            ...
            for tag in speciesEntry['apparelTags']:
                ...
                ele.append(li)
            pawnKindBeingCreated.append(ele)
        if 'xenoType' in speciesEntry and len(speciesEntry['xenoType']) > 0:
